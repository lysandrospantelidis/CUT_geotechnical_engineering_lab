...
def cut_profile(mode, layers, H, beta_deg, alphaH, av, gamma_w, zwt, q,
    ...
    sigma=[]; theta_vals=[]; Kg=[]; Kq=[]; Kc=[]; qterm=[]; cterm=[]; uvals=[]; udvals=[]; svvals=[]; phim_vals=[]; cm_vals=[]
...
class App:
    def __init__(self, root):
        ...
        nb = self.nb
        self.tab_data = ttk.Frame(nb); self.tab_dp = ttk.Frame(nb); self.tab_results = ttk.Frame(nb); self.tab_dist = ttk.Frame(nb)
        nb.add(self.tab_data, text="Point level results"); nb.add(self.tab_results, text="Resultant forces and point of application"); nb.add(self.tab_dist, text="Pressure distributions")
    ...
    def compute(self):
        try:
            ...
            self.tree_active.insert("", "end", values=(
                ...
                _fmt_res4(ena['K'] if (ena and allow_qc_to_en_aashto) else 0.0, nr=ena_nr if allow_qc_to_en_aashto else False),
                _fmt_res4(ena['K'] if (ena and allow_qc_to_en_aashto) else 0.0, nr=ena_nr if allow_qc_to_en_aashto else False),
            ...
            self.tree_active.insert("", "end", values=(
                ...
                _fmt_res4(aask['K'] if (aask and allow_qc_to_en_aashto) else 0.0, nr=aask_nr if allow_qc_to_en_aashto else False),
                _fmt_res4(aask['K'] if (aask and allow_qc_to_en_aashto) else 0.0, nr=aask_nr if allow_qc_to_en_aashto else False),
            ...
            self.tree_passive.insert("", "end", values=(
                ...
                _fmt_res4(enp['K'] if (enp and allow_qc_to_en_aashto) else 0.0, nr=enp_nr if allow_qc_to_en_aashto else False),
                _fmt_res4(enp['K'] if (enp and allow_qc_to_en_aashto) else 0.0, nr=enp_nr if allow_qc_to_en_aashto else False),
            ...
            def populate_z_tree(tree, z_value):
                zq = max(0.0, min(H, float(z_value)))
                ...
                def insert_row(method, sigma_eff, u_val, ud_val, w_val, kq_term, kc_term, sigma_total, kgamma, kq_val, kc_val, nr=False, na=False):
                    tree.insert("", "end", values=(
                        method,
                        _fmt_res3(sigma_eff, nr=nr, na=na),
                        _fmt_res3(u_val, nr=nr, na=na),
                        _fmt_res3(ud_val, nr=nr, na=na),
                        _fmt_res3(kq_term, nr=nr, na=na),
                        _fmt_res3(kc_term, nr=nr, na=na),
                        _fmt_res3(sigma_total, nr=nr, na=na),
                        _fmt_res3(kgamma, nr=nr, na=na),
                        _fmt_res3(kq_val, nr=nr, na=na),
                        _fmt_res3(kc_val, nr=nr, na=na)
                    ))
                ...
                if cuta is not None:
                    ...
                    insert_row("CUT active", cuta["sigma"][iz] - cuta["qterm"][iz] - cuta["u"][iz] - cuta["ud"][iz], cuta["u"][iz], cuta["ud"][iz], 0.0,
                               cuta["qterm"][iz], 0.0, cuta["sigma"][iz], cuta["K_gamma"][iz], cuta["K_q"][iz], 0.0)
                else:
                ...
                if cutp is not None:
                    ...
                    insert_row("CUT passive", cutp["sigma"][iz] - cutp["qterm"][iz] - cutp["u"][iz] - cutp["ud"][iz], cutp["u"][iz], cutp["ud"][iz], 0.0,
                               cutp["qterm"][iz], 0.0, cutp["sigma"][iz], cutp["K_gamma"][iz], cutp["K_q"][iz], 0.0)
                else:
