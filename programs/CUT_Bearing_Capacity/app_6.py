...
def build_summary_table(results: Dict[str, Any]) -> pd.DataFrame:
    ...
    rows = [
        {"Method": "EN 1997-3", "q_ult (kPa)": summary.get("q_ult EN 1997-3 (kPa)"), "R_N (kN)": summary.get("R_N EN 1997-3 (kN)"), "R_N/N": summary.get("Safety ratio EN 1997-3 (R_N/N)")},
        {"Method": "CUT method", "q_ult (kPa)": summary.get("q_ult CUT method (kPa)"), "R_N (kN)": summary.get("R_N CUT method (kN)"), "R_N/N": summary.get("Safety ratio CUT method (R_N/N)")},
        {"Method": "Mobilized shear strength of soil", "q_ult (kPa)": summary.get("q_ult Mobilized shear strength static (kPa)"), "R_N (kN)": summary.get("R_N Mobilized shear strength static (kN)"), "R_N/N": results["Mobilized shear strength of soil"].get("RN/N")},
    ]
...
def build_method_comparison_table(results: Dict[str, Any]) -> pd.DataFrame:
    ...
    def get_val(d: Dict[str, Any], k: str):
        if k == "q_ult":
            return d.get("q_ult", d.get("qult"))
        return d.get(k)
    ...
    for source in (en, cut, mob):
        for k in source.keys():
            nk = normalize_key(k)
            if nk not in ordered_keys:
                ordered_keys.append(nk)

    df = pd.DataFrame({
        "Parameter": ordered_keys,
        "EN 1997-3": [fmt_val(get_val(en, k)) for k in ordered_keys],
        "CUT method": [fmt_val(get_val(cut, k)) for k in ordered_keys],
        "Mobilized shear strength of soil": [fmt_val(get_val(mob, k)) for k in ordered_keys],
    })
    return df
...
with header_cols[1]:
    st.title(APP_TITLE)
    st.caption("Version: v7.1 (Program), v3 (Manual).")
