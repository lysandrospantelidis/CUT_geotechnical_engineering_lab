...
APP_TITLE = "CUT method Bearing Capacity"
SOURCE_FILE = "CUT_Bearing_capacity_updated_v7_1.py"
MANUAL_FILE = "CUT_Bearing_Capacity_User_Manual_v3.pdf"
KOULOMB_FILE = "CUT_K_Coulomb.py"
LOGO_FILE = "cut_logo.png"
BRAND_FILE = "Cut bearing capacity.png"

ABOUT_TEXT = """CUT_Bearing_capacity
Version: v7.1 (Program), v3 (Manual)
...
@st.cache_resource
def load_core_functions(source_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if abs(math.sin(phi_cr)) < eps:
            ...
            s_lg_tot = (r_o / math.sin(phi_cr)) * (
                math.exp((math.pi / 4.0 - phi_cr / 2.0) * math.tan(phi_cr)) - 1.0
            )
        ...
        I_I_phi = sc_influence_integral(phi, footing_type)
...
def compute_all(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def qu_smf(smf: float):
        ...
        qu = cyprus_qult_with_params(c_m, phi_m, 0.0, 0.0, params)
        return float(f"{qu:.3f}"), c_m, phi_m, qu

    best = None
    smf = 1.0
    while smf >= -1e-12:
        qu_r, c_m, phi_m, qu_val = qu_smf(max(0.0, smf))
        err = abs(qu_r - target_qu)
        if best is None or err < best[0] - 1e-12 or (abs(err - best[0]) < 1e-12 and smf > best[1]):
            best = (err, max(0.0, smf), c_m, phi_m, qu_val, qu_r)
        smf = round(smf - 0.0005, 10)
    _, smf_best, cm_best, phim_best, qu_best, qu_r_best = best

    qu_full, mob = cyprus_qult_with_params(cm_best, phim_best, 0.0, 0.0, params, return_factors=True)
    ...
    return {
        "Summary": {
            ...
            "q_ult EN 1997-3 1997-3 (kPa)": None if en_out is None else en_out["qult"],
            "q_ult CUT method method (kPa)": qult_c,
            "q_ult Mobilized shear strength static (kPa)": qu_full,
            "R_N EN 1997-3 1997-3 (kN)": RN_en,
            "R_N CUT method method (kN)": RN_cut,
        ...
        "EN 1997-3 1997-3": {"status": en_status, "compressibility basis": en_comment, **({} if en_out is None else en_out)},
        "CUT method method": cut_factors,
        "Mobilized shear strength shear strength of soil": {"SMF": smf_best, "c_m (kPa)": cm_best, "φ_m (deg)": phim_best, **mob},
        "equivalent_soil": {"c_eq (kPa)": c_val, "φ_eq (deg)": phi, "γ_eq (kN/m³)": gd, "ψ_eq (deg)": psi, "regime": eq_info.get("regime", "-")},
        "eq_info": eq_info,
    }
...
def build_summary_table(results: Dict[str, Any]) -> pd.DataFrame:
    ...
    rows = [
        {"Method": "EN 1997-3 1997-3", "q_ult (kPa)": summary.get("q_ult EN 1997-3 1997-3 (kPa)"), "R_N (kN)": summary.get("R_N EN 1997-3 1997-3 (kN)"), "R_N/N": summary.get("Safety ratio EN 1997-3 (R_N/N)")},
        {"Method": "CUT method method", "q_ult (kPa)": summary.get("q_ult CUT method method (kPa)"), "R_N (kN)": summary.get("R_N CUT method method (kN)"), "R_N/N": summary.get("Safety ratio CUT method (R_N/N)")},
        {"Method": "Mobilized shear strength shear strength of soil", "q_ult (kPa)": summary.get("q_ult Mobilized shear strength static (kPa)"), "R_N (kN)": summary.get("R_N Mobilized shear strength static (kN)"), "R_N/N": results["Mobilized shear strength shear strength of soil"].get("RN/N")},
    ]
    df = pd.DataFrame(rows)
    for col in ["q_ult (kPa)", "R_N (kN)", "R_N/N"]:
        df[col] = df[col].map(fmt_val)
    return df


def build_method_comparison_table(results: Dict[str, Any]) -> pd.DataFrame:
    en = results.get("EN 1997-3 1997-3", {}) or {}
    cut = results.get("CUT method method", {}) or {}
    mob = results.get("Mobilized shear strength shear strength of soil", {}) or {}
    ordered_keys = []
    for source in (en, cut, mob):
        for k in source.keys():
            if k not in ordered_keys:
                ordered_keys.append(k)
    df = pd.DataFrame({
        "Parameter": ordered_keys,
        "EN 1997-3 1997-3": [fmt_val(en.get(k)) for k in ordered_keys],
        "CUT method method": [fmt_val(cut.get(k)) for k in ordered_keys],
        "Mobilized shear strength shear strength of soil": [fmt_val(mob.get(k)) for k in ordered_keys],
    })
    return df
...
with header_cols[1]:
    st.title(APP_TITLE)
    st.caption("Version: v7.1 (Program), v3 (Manual). This is the web version.")

with st.sidebar:
    if Path(BRAND_FILE).exists():
        st.image(BRAND_FILE, use_container_width=True)
    ...
    if Path(MANUAL_FILE).exists():
        st.download_button("Download User Manual (PDF)", data=Path(MANUAL_FILE).read_bytes(), file_name=MANUAL_FILE, mime="application/pdf", use_container_width=True)
    if Path(KOULOMB_FILE).exists():
        st.download_button("Download CUT_K_Coulomb.py", data=Path(KOULOMB_FILE).read_bytes(), file_name=KOULOMB_FILE, mime="text/x-python", use_container_width=True)
    if Path(SOURCE_FILE).exists():
        st.download_button("Download source program", data=Path(SOURCE_FILE).read_bytes(), file_name=SOURCE_FILE, mime="text/x-python", use_container_width=True)
...
with st.container():
    ...
    with c1:
        ...
        allow_en_special = st.checkbox("Allow EN 1997-3 for 2-layer / seismic using c_m and φ_m", value=False)

    with c2:
        st.subheader("Geometry & loading")
        Beff = num("B, B′ or D (m)", 2.0)
        Leff = num("L or L′ (m)", 2.0)
        Df = num("Df = DfP (m)", 1.0)
        DfA = num("DfA (m)", 1.0)
    ...
    with c3:
        ...
        k_source = st.selectbox("Source for KA & KP (CUT method method)", ["B_EN", "B_AP", "KAKP"], format_func=lambda x: {"B_EN": "β same with EN", "B_AP": "βA & βP", "KAKP": "KA & KP"}[x])
        if k_source == "B_AP":
...
if "results" in st.session_state:
    ...
    with m1:
        st.metric("CUT method method q_ult (kPa)", fmt_val(results["Summary"]["q_ult CUT method method (kPa)"]))
    with m2:
        st.metric("EN 1997-3 1997-3 q_ult (kPa)", fmt_val(results["Summary"]["q_ult EN 1997-3 1997-3 (kPa)"]))
    with m3:
        st.metric("Mobilized shear strength shear strength q_ult (kPa)", fmt_val(results["Summary"]["q_ult Mobilized shear strength static (kPa)"]))
    ...
    with tabs[2]:
        eq_df = pd.DataFrame({
            "Parameter": list(results["equivalent_soil"].keys()) + list(results["eq_info"].keys()),
            "Value": [fmt_val(v) for v in results["equivalent_soil"].values()] + [fmt_val(v) for v in results["eq_info"].values()],
        })
        st.dataframe(eq_df, use_container_width=True, hide_index=True)
    with tabs[3]:
        st.write("The web app follows the final v7.1 program and v3 manual, including the original EN 1997-3 checkbox logic, βA/βP and KA/KP inputs, and CUT_K_Coulomb as an auxiliary download tool.")
