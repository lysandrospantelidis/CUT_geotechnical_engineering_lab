...
def find_manual_file(item: dict):
    ...
    for entry in entries:
        ...
        if not name_l.endswith(".pdf"):
            continue
        if file_contains and file_contains.lower() not in name_l:
            continue
...
def check_url_status(url: str):
    """
    Lightweight deployment checker for public web apps.
    Tries HEAD first, then GET if needed.
    """
    if not url:
        return {"ok": False, "status": None, "detail": "Missing URL"}

    headers = {"User-Agent": "CUT-Geotechnical-Hub"}
    try:
        req = Request(url, headers=headers, method="HEAD")
        with urlopen(req, timeout=10) as resp:
            code = getattr(resp, "status", 200)
            return {"ok": 200 <= code < 400, "status": code, "detail": "HEAD"}
    except Exception:
        try:
            req = Request(url, headers=headers, method="GET")
            with urlopen(req, timeout=12) as resp:
                code = getattr(resp, "status", 200)
                return {"ok": 200 <= code < 400, "status": code, "detail": "GET"}
        except HTTPError as e:
            return {"ok": False, "status": e.code, "detail": "HTTPError"}
    ...
    with st.expander(header_text, expanded=False):
        ...
        if mode == "web":
            ...
            if app_url:
                ...
                status = check_url_status(app_url)
                if status["ok"]:
                    st.success(f"Deployment check: online ({status['status']})")
                else:
                    detail = f" ({status['status']})" if status["status"] else ""
                    st.warning(f"Deployment check: not confirmed{detail}")
    ...
    with st.expander("🛠 Latest updates", expanded=True):
        ...
        if public_releases:
            for rel in public_releases:
                ...
                title_line = f"**{date}** — {name}" if date else f"**{name}**"
                st.markdown(title_line)

                if body:
                    first_paragraph = body.split("\n\n")[0].strip()
                    if len(first_paragraph) > 350:
                        first_paragraph = first_paragraph[:350].rstrip() + "..."
                    st.markdown(first_paragraph)

                if rel.get("html_url"):
                    st.link_button(
                        f"Open release: {tag or name}",
                        rel["html_url"],
                        use_container_width=True,
                    )
...
def build_share_links(url: str):
    ...
    encoded_mail_body = quote(url, safe="")
...
with tab6:
    st.markdown(
        '<div class="section-intro">Contact information, academic profiles, and sharing options.</div>',
        unsafe_allow_html=True,
    )
    ...
    with left:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div class="card-title">Contact & Profiles</div>', unsafe_allow_html=True)
    ...
    with right:
        ...
        if SHARE_URL:
            ...
            for label, url in share_links.items():
                st.link_button(f"Share on {label}", url, use_container_width=True)
