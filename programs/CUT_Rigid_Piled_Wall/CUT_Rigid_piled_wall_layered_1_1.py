# CUT_Rigid_piled_wall_layered.py
...
@dataclass
class SoilLayer:
    # z_bottom is local depth below the ground surface of the corresponding side.
    z_bottom: float
    gamma: float
    gamma_sat: float
    c_prime: float
...
def _sorted_layers(side_data: SideData) -> list[SoilLayer]:
    layers = [ly for ly in getattr(side_data, "layers", []) if ly.z_bottom > 0.0]
    ...
    if not layers:
        return [
            SoilLayer(
                ...
                side_data.gamma_sat,
                side_data.c_prime,
                side_data.phi_prime_deg,
                side_data.E_s,
                side_data.nu,
            )
        ]

    layers = sorted(layers, key=lambda ly: float(ly.z_bottom))

    last = layers[-1]
    if last.z_bottom < 1e29:
        layers.append(
            SoilLayer(
                1e30,
                last.gamma,
...
def layer_at_depth(side_data: SideData, z_local_from_ground: float) -> SoilLayer:
    zq = max(0.0, float(z_local_from_ground))

    for ly in _sorted_layers(side_data):
        if zq <= float(ly.z_bottom) + 1e-9:
            return ly

    return _sorted_layers(side_data)[-1]


def side_data_at_depth(side_data: SideData, z_local_from_ground: float) -> SideData:
    ly = layer_at_depth(side_data, z_local_from_ground)
    ...
    return SideData(
        ...
        z_w=side_data.z_w,
        gamma=ly.gamma,
        gamma_sat=ly.gamma_sat,
        c_prime=_regularize_c_prime(ly.c_prime),
        phi_prime_deg=_regularize_phi_prime_deg(ly.phi_prime_deg),
        E_s=ly.E_s,
        nu=ly.nu,
        layers=side_data.layers,
    )

def layered_vertical_geostatic_stress(
    z_local: float,
    side_data: SideData,
    z_w_local: float,
    gamma_w: float = 0.0,
    ...
    if z_local <= 0.0:
        return gamma_w * (-z_w_local) if include_water_overburden and z_w_local < 0.0 else 0.0
    total = gamma_w * (-z_w_local) if include_water_overburden and z_w_local < 0.0 else 0.0
    z0 = 0.0
    for ly in _sorted_layers(side_data):
        z1 = min(float(z_local), ly.z_bottom)
        if z1 <= z0:
            continue
        if z1 <= z_w_local:
            total += ly.gamma * (z1 - z0)
        elif z0 >= z_w_local:
            total += ly.gamma_sat * (z1 - z0)
        else:
            total += ly.gamma * (z_w_local - z0) + ly.gamma_sat * (z1 - z_w_local)
        z0 = z1
        if z0 >= z_local:
            break
    return total
...
def layered_vertical_interval_stress(
    z_start: float,
    z_end: float,
    side_data: SideData,
    z_w_local: float,
) -> float:
    """Integrate soil unit weight only between two local depths below ground."""
    z_start = max(0.0, float(z_start))
    z_end = max(z_start, float(z_end))
    if z_end <= z_start:
        return 0.0
    total = 0.0
    z0 = 0.0
    for ly in _sorted_layers(side_data):
        seg0 = max(z_start, z0)
        seg1 = min(z_end, ly.z_bottom)
        if seg1 > seg0:
            if seg1 <= z_w_local:
                total += ly.gamma * (seg1 - seg0)
            elif seg0 >= z_w_local:
                total += ly.gamma_sat * (seg1 - seg0)
            else:
                total += ly.gamma * (z_w_local - seg0) + ly.gamma_sat * (seg1 - z_w_local)
        z0 = ly.z_bottom
        if z0 >= z_end:
            break
    return total
...
def compute_q_RA_total(geometry: Geometry, right_data: SideData, gamma_w: float) -> float:
    ...
    sigma_v_geo_base = layered_vertical_geostatic_stress(
...
def total_vertical_stress(
    ...
    if quadrant in ("RA", "LP"):
        sigma_v_geo = layered_vertical_geostatic_stress(
            z_local=z_local,
            side_data=side_data,
            z_w_local=z_w_local,
            gamma_w=gamma_w,
        ...
        # Lower right quadrant: q_upper carries the overburden down to z_p;
        # add only the layered increment from z_p to the current depth.
        sigma_v_geo = layered_vertical_interval_stress(
            z_start=geometry.z_p,
            z_end=z,
            side_data=side_data,
            z_w_local=side_data.z_w,
        )
    elif quadrant == "LA":
        z_top_L = geometry.H_R - geometry.H_L
        H_LP = geometry.z_p - z_top_L
        if H_LP > 0.0:
            sigma_v_geo = layered_vertical_interval_stress(
                z_start=H_LP,
                z_end=z - z_top_L,
                side_data=side_data,
                z_w_local=side_data.z_w - z_top_L,
            )
        else:
            sigma_v_geo = layered_vertical_interval_stress(
                z_start=0.0,
...
def solve_point(
    ...
    side_data_base = get_side_data(quadrant, left_data, right_data)
    z_local = local_depth(z, quadrant, geometry.H_R, geometry.H_L, geometry.z_p)
    side_for_point = quadrant_side(quadrant)
    if side_for_point is None:
        raise ValueError("Cannot select soil layer for void quadrant.")

    z_from_ground = side_mobilization_depth(
        z,
        side_for_point,
        geometry.H_R,
        geometry.H_L,
    ...
    side_data = side_data_at_depth(side_data_base, z_from_ground)
...
class CutSolverApp(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("CUT_Rigid_piled_wall_layered")
    ...
    def _build_variables(self) -> None:
        ...
        self.left_layer_rows: list[dict[str, tk.Variable]] = []
        self.right_layer_rows: list[dict[str, tk.Variable]] = []
        self.var_left_layer_selected = tk.IntVar(value=0)
        self.var_right_layer_selected = tk.IntVar(value=0)
    ...
    def _load_defaults(self) -> None:
        ...
        self.left_layer_rows = [self._layer_to_vars(SoilLayer(g.H_L, l.gamma, l.gamma_sat, l.c_prime, l.phi_prime_deg, l.E_s, l.nu))]
        self.right_layer_rows = [self._layer_to_vars(SoilLayer(g.H_R, r.gamma, r.gamma_sat, r.c_prime, r.phi_prime_deg, r.E_s, r.nu))]
        self.var_left_layer_selected.set(0)
        self.var_right_layer_selected.set(0)
        self._refresh_layer_grids()
    ...
    def _build_layout(self) -> None:
        ...
        self.left_canvas = tk.Canvas(left_outer, width=360, highlightthickness=0)
        self.left_canvas.grid(row=0, column=0, sticky="nsew")
        left_scroll = ttk.Scrollbar(left_outer, orient="vertical", command=self.left_canvas.yview)
        left_scroll.grid(row=0, column=1, sticky="ns")
        left_scroll_x = ttk.Scrollbar(left_outer, orient="horizontal", command=self.left_canvas.xview)
        left_scroll_x.grid(row=1, column=0, sticky="ew")
        self.left_canvas.configure(yscrollcommand=left_scroll.set, xscrollcommand=left_scroll_x.set)
    ...
    def _layer_to_vars(self, layer: SoilLayer) -> dict[str, tk.Variable]:
        return {
            "z_bottom": tk.DoubleVar(value=layer.z_bottom),
            "gamma": tk.DoubleVar(value=layer.gamma),
            "gamma_sat": tk.DoubleVar(value=layer.gamma_sat),
            "c_prime": tk.DoubleVar(value=layer.c_prime),
            "phi_prime_deg": tk.DoubleVar(value=layer.phi_prime_deg),
            "E_s": tk.DoubleVar(value=layer.E_s),
            "nu": tk.DoubleVar(value=layer.nu),
        }

    def _build_layer_grid(self, parent: ttk.Frame, side: SideName) -> None:
        rows = self.left_layer_rows if side == "left" else self.right_layer_rows
        selected = self.var_left_layer_selected if side == "left" else self.var_right_layer_selected
        for w in parent.winfo_children():
            w.destroy()
        title = "Left side layers" if side == "left" else "Right side layers"
        ttk.Label(parent, text=title, font=("TkDefaultFont", 9, "bold")).grid(row=0, column=0, columnspan=8, sticky="w", pady=(2, 4))
        headers = ["", "z\nbottom\n(m)", "γ\n(kN/m³)", "γsat\n(kN/m³)", "c'\n(kPa)", "φ'\n(deg)", "E_s\n(kPa)", "ν\n(-)"]
        widths = [2, 6, 6, 6, 5, 5, 7, 5]
        for col, h in enumerate(headers):
            ttk.Label(parent, text=h, anchor="center").grid(row=1, column=col, sticky="ew", padx=2, pady=1)
        for i, row_vars in enumerate(rows):
            ttk.Radiobutton(parent, variable=selected, value=i).grid(row=i + 2, column=0, sticky="w", padx=(0, 2))
            for col, key in enumerate(["z_bottom", "gamma", "gamma_sat", "c_prime", "phi_prime_deg", "E_s", "nu"], start=1):
                ttk.Entry(parent, textvariable=row_vars[key], width=widths[col]).grid(row=i + 2, column=col, sticky="w", padx=2, pady=2)
        btn_row = len(rows) + 2
        ttk.Button(parent, text="Add layer", command=lambda s=side: self._add_layer_row(s)).grid(row=btn_row, column=1, columnspan=2, sticky="ew", padx=2, pady=(6, 2))
        ttk.Button(parent, text="Remove selected", command=lambda s=side: self._remove_selected_layer_row(s)).grid(row=btn_row, column=3, columnspan=3, sticky="ew", padx=2, pady=(6, 2))

    def _refresh_layer_grids(self) -> None:
        if hasattr(self, "left_layers_grid"):
            self._build_layer_grid(self.left_layers_grid, "left")
        if hasattr(self, "right_layers_grid"):
            self._build_layer_grid(self.right_layers_grid, "right")

    def _add_layer_row(self, side: SideName) -> None:
        rows = self.left_layer_rows if side == "left" else self.right_layer_rows
        selected = self.var_left_layer_selected if side == "left" else self.var_right_layer_selected
        if rows:
            last = rows[-1]
            new_layer = SoilLayer(float(last["z_bottom"].get()) + 1.0, float(last["gamma"].get()), float(last["gamma_sat"].get()), float(last["c_prime"].get()), float(last["phi_prime_deg"].get()), float(last["E_s"].get()), float(last["nu"].get()))
        else:
            H = self.var_H_L.get() if side == "left" else self.var_H_R.get()
            new_layer = SoilLayer(H, 18.0, 20.0, 1.0, 30.0, 20000.0, 0.30)
        rows.append(self._layer_to_vars(new_layer))
        selected.set(len(rows) - 1)
        self._refresh_layer_grids()

    def _remove_selected_layer_row(self, side: SideName) -> None:
        rows = self.left_layer_rows if side == "left" else self.right_layer_rows
        selected = self.var_left_layer_selected if side == "left" else self.var_right_layer_selected
        if len(rows) <= 1:
            return
        idx = max(0, min(len(rows) - 1, int(selected.get())))
        rows.pop(idx)
        selected.set(max(0, min(len(rows) - 1, idx)))
        self._refresh_layer_grids()

    def _read_layers(self, side: SideName, H_side: float) -> list[SoilLayer]:
        rows = self.left_layer_rows if side == "left" else self.right_layer_rows
        layers: list[SoilLayer] = []
        for row in rows:
            layers.append(SoilLayer(round(float(row["z_bottom"].get()), 1), float(row["gamma"].get()), float(row["gamma_sat"].get()), _regularize_c_prime(float(row["c_prime"].get())), float(row["phi_prime_deg"].get()), round(float(row["E_s"].get()), 0), float(row["nu"].get())))
        if not layers:
            layers.append(SoilLayer(H_side, 18.0, 20.0, 1.0, 30.0, 20000.0, 0.30))
        layers = sorted(layers, key=lambda ly: ly.z_bottom)
        if layers[-1].z_bottom < H_side:
            last = layers[-1]
            layers[-1] = SoilLayer(H_side, last.gamma, last.gamma_sat, last.c_prime, last.phi_prime_deg, last.E_s, last.nu)
        return layers
    ...
    def _build_input_panel(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(input_box, text="γ_w (kN/m³)").grid(row=5, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(input_box, textvariable=self.var_gamma_w, width=8).grid(row=5, column=1, sticky="w", padx=4, pady=2)

        layers_box = ttk.LabelFrame(parent, text="Soil layers")
        layers_box.grid(row=1, column=0, columnspan=2, sticky="ew", pady=4)
        self.left_layers_grid = ttk.Frame(layers_box)
        self.left_layers_grid.grid(row=0, column=0, sticky="ew", padx=4, pady=(2, 8))
        self.right_layers_grid = ttk.Frame(layers_box)
        self.right_layers_grid.grid(row=1, column=0, sticky="ew", padx=4, pady=(2, 4))
        self._refresh_layer_grids()

        seis = ttk.LabelFrame(parent, text="Seismic coefficients")
        seis.grid(row=2, column=0, columnspan=2, sticky="ew", pady=4)
        ...
        move.grid(row=3, column=0, columnspan=2, sticky="ew", pady=4)
        ...
        ttk.Checkbutton(move, text="Auto equilibrium search", variable=self.var_auto).grid(row=3, column=0, columnspan=4, sticky="w", padx=4, pady=2)
        ...
        misc.grid(row=4, column=0, columnspan=2, sticky="ew", pady=4)
        self._add_entry(misc, 0, "query z (m, red dot)", self.var_z_query)
        self._add_entry(misc, 1, "n profile points (-)", self.var_n_points)
        btns = ttk.Frame(parent)
        btns.grid(row=5, column=0, columnspan=2, sticky="ew", pady=8)
    ...
    def _build_output_panel(self, parent: ttk.Frame) -> None:
        ...
        self.tree_res = ttk.Treeview(
            result_frame,
            columns=("side", "quad", "z_upper", "z_lower", "dir", "F", "Fe", "M", "Me", "mdir", "z", "W"),
            show="headings",
            height=6,
        )
        ...
        widths = {
            ...
            "dir": 75, "F": 105, "Fe": 110, "M": 120, "Me": 110, "mdir": 90, "z": 90, "W": 110
        }
    ...
    def collect_inputs(self) -> tuple[Geometry, SideData, SideData, SeismicData, MovementData, float, float, int, float]:
        ...
        left_layers = self._read_layers("left", geometry.H_L)
        right_layers = self._read_layers("right", geometry.H_R)
        l0 = left_layers[0]
        r0 = right_layers[0]
        ...
        right_data = SideData(
            ...
            z_w=z_w_R_input,
            gamma=r0.gamma,
            gamma_sat=r0.gamma_sat,
            c_prime=_regularize_c_prime(r0.c_prime),
            phi_prime_deg=r0.phi_prime_deg,
            E_s=r0.E_s,
            nu=r0.nu,
            layers=right_layers,
        )
        left_data = SideData(
            ...
            z_w=z_w_L_input,
            gamma=l0.gamma,
            gamma_sat=l0.gamma_sat,
            c_prime=_regularize_c_prime(l0.c_prime),
            phi_prime_deg=l0.phi_prime_deg,
            E_s=l0.E_s,
            nu=l0.nu,
            layers=left_layers,
        )
    ...
    def _adverse_label(self, value: float, adverse_when_positive: bool) -> str:
        """Return a Greek effect label based on the requested sign convention."""
        if abs(value) <= 1e-9:
            return "—"
    ...
    def update_pressure_diagram(
        ...
        for side, quad, direction, res in results:
            ...
            self.tree_res.insert(
                ...
                values=(
                    ...
                    direction,
                    f"{res['F']:.2f}",
                    self._adverse_label(res["F"], adverse_when_positive=True),
                    f"{res['M']:.2f}",
                    self._adverse_label(res["M"], adverse_when_positive=False),
        ...
        self.tree_res.insert(
            ...
            values=(
                ...
                "",
                f"{sum_F:.2f}",
                self._adverse_label(sum_F, adverse_when_positive=True),
                f"{sum_M:.2f}",
                self._adverse_label(sum_M, adverse_when_positive=False),
