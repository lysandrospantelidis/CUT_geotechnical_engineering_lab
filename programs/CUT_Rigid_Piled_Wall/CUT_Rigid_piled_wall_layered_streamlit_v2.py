# CUT_Rigid_piled_wall_layered_streamlit.py
...
APP_NAME = "CUT_Rigid_piled_wall_layered"
...
def _example_setup() -> tuple[Geometry, SideData, SideData, SeismicData, MovementData, float]:
    ...
    right_data = SideData(5.0, 20.0, 3.0, 18.0, 20.0, 5.0, 30.0, 20000.0, 0.30,
                          [SoilLayer(10.0, 18.0, 20.0, 5.0, 30.0, 20000.0, 0.30)])
    left_data = SideData(3.0, 10.0, 3.0, 17.5, 19.5, 4.0, 28.0, 18000.0, 0.32,
                         [SoilLayer(7.0, 17.5, 19.5, 4.0, 28.0, 18000.0, 0.32)])
...
def _make_layers(prefix: str, H_side: float, default_side: SideData) -> list[SoilLayer]:
    st.markdown(f"**{prefix.capitalize()} side layers**")
    n_layers = st.number_input(
        f"Number of {prefix} layers",
        min_value=1,
        max_value=8,
        value=max(1, len(default_side.layers) if default_side.layers else 1),
        step=1,
        key=f"{prefix}_n_layers",
    )
    layers: list[SoilLayer] = []
    base_layers = default_side.layers or [SoilLayer(H_side, default_side.gamma, default_side.gamma_sat, default_side.c_prime, default_side.phi_prime_deg, default_side.E_s, default_side.nu)]
    for i in range(int(n_layers)):
        base = base_layers[min(i, len(base_layers) - 1)]
        with st.expander(f"{prefix.capitalize()} layer {i + 1}", expanded=(i == 0)):
            c1, c2 = st.columns(2)
            with c1:
                z_bottom = st.number_input("z bottom (m)", value=float(base.z_bottom if i < len(base_layers) else H_side), step=0.1, key=f"{prefix}_{i}_z")
                gamma = st.number_input("γ (kN/m³)", value=float(base.gamma), step=0.1, key=f"{prefix}_{i}_gamma")
                gamma_sat = st.number_input("γsat (kN/m³)", value=float(base.gamma_sat), step=0.1, key=f"{prefix}_{i}_gamma_sat")
                c_prime = st.number_input("c' (kPa)", value=float(base.c_prime), step=0.1, key=f"{prefix}_{i}_c")
            with c2:
                phi = st.number_input("φ' (deg)", value=float(base.phi_prime_deg), step=0.1, key=f"{prefix}_{i}_phi")
                E_s = st.number_input("E_s (kPa)", value=float(base.E_s), step=100.0, key=f"{prefix}_{i}_E")
                nu = st.number_input("ν (-)", value=float(base.nu), min_value=0.0, max_value=0.49, step=0.01, key=f"{prefix}_{i}_nu")
        layers.append(SoilLayer(round(float(z_bottom), 3), float(gamma), float(gamma_sat), _regularize_c_prime(float(c_prime)), float(phi), float(E_s), float(nu)))
...
def main():
    ...
    with st.sidebar:
        ...
        left_layers = _make_layers("left", float(H_L), left_d)
        right_layers = _make_layers("right", float(H_R), right_d)
        l0, r0 = left_layers[0], right_layers[0]
        left_data = SideData(float(beta_L), float(q_L), float(z_w_L), l0.gamma, l0.gamma_sat, l0.c_prime, l0.phi_prime_deg, l0.E_s, l0.nu, left_layers)
        right_data = SideData(float(beta_R), float(q_R), float(z_w_R), r0.gamma, r0.gamma_sat, r0.c_prime, r0.phi_prime_deg, r0.E_s, r0.nu, right_layers)
