...
def reset_model_state() -> None:
    ...
    st.session_state.soil_gamma = 18.0
    st.session_state.soil_ks = 28000.0
    st.session_state.beam_gamma = 20.0
    st.session_state.beam_E = 21_000_000.0
    st.session_state.beam_nu = 0.15

    st.session_state.section_type = "Trapezoidal"
    st.session_state.sec_b = 2.0
    st.session_state.sec_h = 2.0
    st.session_state.sec_b0 = 0.5
    st.session_state.sec_h0 = 0.5
    st.session_state.sec_h1 = 0.5
    st.session_state.sec_h2 = 0.5
    st.session_state.sec_A = 1.0
    st.session_state.sec_I = 1.0

    st.session_state.nodes_df = pd.DataFrame({"x [m]": [0.0, 5.0, 10.0]})
    st.session_state.nodal_loads_df = pd.DataFrame(columns=["node", "V [kN]", "M [kNm]"])
    st.session_state.beam_loads_df = pd.DataFrame(columns=["beam", "pA [kN/m]", "pB [kN/m]"])
...
def build_input_summary_dataframe() -> pd.DataFrame:
    rows = [
        ["Project", st.session_state.project],
        ["User", st.session_state.user],
        ["Comments", st.session_state.comments],
        ["Soil γ", round(float(st.session_state.soil_gamma), 2)],
        ["Soil Ks", round(float(st.session_state.soil_ks), 2)],
        ["Beam γ", round(float(st.session_state.beam_gamma), 2)],
        ["Beam E", round(float(st.session_state.beam_E), 2)],
        ["Beam ν", round(float(st.session_state.beam_nu), 2)],
        ["Section type", st.session_state.section_type],
        ["b", round(float(st.session_state.sec_b), 2)],
        ["h", round(float(st.session_state.sec_h), 2)],
    ]

    if st.session_state.section_type == "Trapezoidal":
        rows.extend([
            ["b0", round(float(st.session_state.sec_b0), 2)],
            ["h0", round(float(st.session_state.sec_h0), 2)],
            ["h1", round(float(st.session_state.sec_h1), 2)],
            ["h2", round(float(st.session_state.sec_h2), 2)],
        ])
        ...
        rows.extend([
            ["A", round(float(st.session_state.sec_A), 2)],
            ["I", round(float(st.session_state.sec_I), 2)],
        ])

    return pd.DataFrame(rows, columns=["Input", "Value"])


def _table_figure(df: pd.DataFrame, title: str, figsize=(11.69, 8.27)):
...
def export_pdf_bytes(domain_for_view: Domain | None, solved_domain: Domain | None) -> bytes:
    buffer = BytesIO()

    with PdfPages(buffer) as pdf:
        # Input summary
        fig = _table_figure(build_input_summary_dataframe(), "Input summary")
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Nodes
        fig = _table_figure(round_df(clean_nodes_df(st.session_state.nodes_df)), "Nodes")
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Nodal loads
        fig = _table_figure(round_df(clean_nodal_loads_df(st.session_state.nodal_loads_df)), "Nodal loads")
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Beam loads
        fig = _table_figure(round_df(clean_beam_loads_df(st.session_state.beam_loads_df)), "Beam loads")
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Geometry
        if domain_for_view is not None:
            fig = plot_geometry(domain_for_view)
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

        # Solved diagrams + results table
        if solved_domain is not None:
            fig = plot_result(solved_domain, ResultsType.U, "Displacement", "u [m]")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

            fig = plot_result(solved_domain, ResultsType.M, "Moment", "M [kNm]")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

            fig = plot_result(solved_domain, ResultsType.V, "Shear", "V [kN]")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

            fig = plot_result(solved_domain, ResultsType.P, "Soil pressure", "p [kPa]")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

            fig = _table_figure(results_dataframe(solved_domain), "Results table")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)
...
with st.sidebar:
    ...
    if c2.button("About", use_container_width=True):
        st.session_state.show_about = not st.session_state.show_about

    st.link_button("CUT-APPS", "https://cut-apps.streamlit.app/", use_container_width=True)

    # Manual button
    manual_path = Path(__file__).resolve().parent / "manual.pdf"
    if manual_path.exists():
        ...
        st.button("📖 Manual (file not found)", disabled=True, use_container_width=True)
    ...
    if export_domain is not None:
        st.download_button(
            "Save",
            data=export_domain.to_xml().encode("utf-8"),
            file_name="jwinkler_model.xml",
            mime="application/xml",
            use_container_width=True,
        )

    domain_for_pdf = st.session_state.solved_domain
    if domain_for_pdf is None:
        try:
            domain_for_pdf = build_domain()
        except Exception:
            domain_for_pdf = None

    pdf_bytes = export_pdf_bytes(domain_for_pdf, st.session_state.solved_domain)
    st.download_button(
        "Export PDF",
        data=pdf_bytes,
        file_name="jwinkler_report.pdf",
        mime="application/pdf",
        use_container_width=True,
    )
